...
create table public.tickets (
  ...
  -- Input metadata
  input_type text, -- 'text', 'voice', 'file', 'combined'
  original_content jsonb, -- Store original voice/file references
...
create table public.ticket_activities (
  ...
  activity_type text not null, -- 'comment', 'status_change', 'assignment', 'resolution'
  content text,
  metadata jsonb,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
-- Functions for automatic ticket numbering
...
begin
  ...
  values (new.id, 'worker', coalesce(new.raw_user_meta_data->>'name', new.email));
  return new;
end;
$$ language plpgsql security definer;
